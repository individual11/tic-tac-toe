...
const makeMove: PlayerMoveFunction = (xPositions, oPositions, mySymbol) => {
	...
	// Try to block a winning line
	for (const combo of winningCombos) {
		const enemyMovesInCombo = combo.filter(pos => enemyPositions.includes(pos));
		if (enemyMovesInCombo.length === 2) {
				const blockingMove = combo.find(pos => !enemyPositions.includes(pos) && !myPositions.includes(pos));
